...
import { FaCertificate, FaExternalLinkAlt, FaInfoCircle } from "react-icons/fa";
...
const Formacion = () => {
  ...
  const academia = [
    {
      ...
      institucion: "Universidad Nacional del Nordeste - FACENA",
      fecha: "Egreso: 2025",
      estado: "GRADUADO",
    ...
    },
    {
      ...
      descripcion: "Formación de grado enfocada en gestión y arquitectura. Actualmente desarrollando tesina sobre IA aplicada al sector bancario.",
      credencialUrl: "#", 
      logoCard: unneLogo 
    }
  ];
  ...
  const certificaciones = [
    ...
    {
      id: 3,
      titulo: "Talentos Digitales programación Web Full Stack",
      institucion: "TeLCO SAPEM",
      fecha: "Dic 2025",
    ...
    {
      id: 4,
      titulo: "Bases de datos Inteligentes",
      institucion: "IBM",
      fecha: "Dic 2024",
  ...
  return (
    <div className="education-page">
      ...
      <Container className="education-container">
              ...
              <Col lg={6} key={item.id}>
                <div className="academic-high-card">
                  ...
                  <div className="academic-logo-wrapper"> 
                    <img src={item.logoCard} alt="Logo Institución" className="academic-logo-img" />
                  </div>
                  
                  <div className="academic-info">
                    <Badge className="status-badge">{item.estado}</Badge>
                    <h3>{item.titulo}</h3>
                    <h5>{item.institucion}</h5>
                    <p className="academic-desc">{item.descripcion}</p>
                    {item.credencialUrl !== "#" && (
        ...
        <hr className="section-divider" style={{ opacity: 0.1, margin: '60px 0' }} />
            ...
            <Col key={cert.id} lg={6}>
              <div className="education-card">
                ...
                <div className="cert-content-side">
                  ...
                  <p className="cert-desc-card">{cert.descripcion}</p>
              ...
              <Row className="align-items-center g-4">
                <Col lg={7}>
                  <div className="modal-preview-container">
                    {/* Lógica para detectar si es el comprobante PDF de la UNNE */}
                    {selectedCert.id === "uni-1" ? (
                      <object data={selectedCert.credencialUrl} type="application/pdf" width="100%" height="500px">
                        <div className="p-4 text-center">
                          <p className="text-white">El navegador no puede mostrar el PDF.</p>
                          <a href={selectedCert.credencialUrl} className="btn-verify-modal" download>Descargar Comprobante</a>
                        </div>
                      </object>
                    ) : (
                      <img src={selectedCert.imagen || selectedCert.logoCard} alt="Certificado" className="img-fluid rounded" />
                    )}
                  </div>
                </Col>
                <Col lg={5}>
                  <h5 className="text-success">{selectedCert.institucion}</h5>
                  <p className="text-secondary">{selectedCert.fecha}</p>
                  ...
                  {selectedCert.credencialUrl !== "#" && (
                    <a href={selectedCert.credencialUrl} target="_blank" rel="noopener noreferrer" className="btn-verify-modal">
                      {selectedCert.id === "uni-1" ? "Abrir Documento Original" : "Ver Credencial Oficial"} <FaExternalLinkAlt />
                    </a>
                  )}
                </Col>
              </Row>
